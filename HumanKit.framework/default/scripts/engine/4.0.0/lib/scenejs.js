...
SceneJS_ChunkFactory.prototype.webglRestored = function () {
    ...
    for (var chunkId in this._chunks) {
        ...
        if (this._chunks.hasOwnProperty(chunkId)) {
            ...
            if (chunk && chunk.build) { // Method augmented on core by user
                chunk.build();
            }
        }
    }
...
SceneJS.configure({ pluginPath: "http://scenejs.org/api/latest/plugins" });;// SceneJS configuration
SceneJS.configure({
