...
.factory('HumanActions', ['$timeout', '$parse', function ($timeout, $parse) {
  var actionsMap = {};

  var doAction = function (action, customContext, callback) {
    if(action.length === 0) {
      $timeout(callback);
      return;
    }

    var _action = action.shift().split(':');
    var _instruction = _action[0];
    var _arguments = _action[1];

    var nextAction = doAction.bind(null, action, customContext, callback);
    // Make async or else can run into chaining problems with engine callbacks
    ...
    if(actionsMap.hasOwnProperty(_instruction)) {
      ...
      var data = $parse(_arguments)(context);

      actionObj.fn(_instruction, data, nextAction);
    } else {
      nextAction();
    }
  };
  ...
  var HumanActions = {
    ...
    define: function (actionNames, fn, context) {
      var action = {
        fn: fn || angular.noop,
        context: context || {}
      };
    ...
    parse: function () {
      ...
      doAction(action.split('|'), customContext, callback);
    }
  };
...
.run(['$timeout', 'Human', 'HumanActions', 'Hotspots', function ($timeout, Human, HumanActions, Hotspots) {
  var context = { chapters: [] };

  Human.events.on('modules.activating', function (data) {
    var chapters = Human.modules.modules[data.moduleId].chapters || [];

    context.chapters.length = 0;

    for (var i = 0; i < chapters.length; i++) {
      context.chapters[i] = chapters[i];
    }
  });

  // Wait action
  var waitAction = function (instruction, data, callback) {
    $timeout(callback, data * 1000);
  };

  HumanActions.define(['wait'], waitAction, context);
  ...
  HumanActions.define(['hideObjects', 'showObjects'], objectsAction, context);
  ...
  HumanActions.define(['flyTo', 'jumpTo'], cameraAction, context);

  // Play action
  var playAction = function (instruction, data, callback) {
    var params = {};
    var bindStopped = true;

    var start, finish;

    if(!angular.isDefined(data)) { // No data, play from wherever we are now

      params.startTime = Human.timeline.time;
    } else if(angular.isNumber(data)) { // Single time
    ...
    } else if(angular.isArray(data)) { // A range
      start = data[0];
      finish = data[1];

      if(!angular.isDefined(start)) { // No data, play from wherever we are now
        params.startTime = Human.timeline.time;
      } else if(angular.isFunction(start)) {
        ...
        params.startChapterId = start.id;
      }

      if(angular.isNumber(finish)) {
        params.finishTime = finish;
      } else if(angular.isFunction(finish)) {

        finish = finish();
        params.finishTime = finish;
      } else {
        params.finishChapterId = finish.id;
      }

      if(angular.isNumber(start) || angular.isNumber(finish)) {
        params.align = false; // Prevent alignment to the entire chapter
      }
    } else if(angular.isObject(data)) { // Single chapter object
      params.startChapterId = data.id;
      params.numChapters = 1;

      // Get animation data, a bit convoluted :(
      var i = Human.timeline.chapters[data.id].index;
      var animation = Human.timeline.chapterList[i].animation;
  ...
  // Special indicator for play commands
  var now = {
    now: function () {
  ...
  HumanActions.define(['play'], playAction, angular.extend(now, context));
  ...
  HumanActions.define(['scrub'], scrubAction, context);
  ...
  HumanActions.define(['stop'], stopAction, context);

  // Hotspots action
  var hotspotsAction = function (instruction, data, callback) {
    ...
    for (var i = 0; i < data.length; i++) {
      var hotspot = Hotspots.getFromSrcData(data[i]);
  ...
  HumanActions
    .define(['hideHotspots', 'showHotspots'], hotspotsAction, context);
}]);
...
// TODO: Hotspots on module level
.factory('Hotspots',
  ['$window', '$document', '$filter', '$interpolate', 'Human', 'HOTSPOT_TYPES', function ($window, $document, $filter, $interpolate, Human, HOTSPOT_TYPES) {

    var Hotspots = {
      srcData: { // Data from the module, should remain unchanged
        all: {},
        byId: {},
        byChapter: {}
      },
      hotspots: [] // Stores current hotspots
    };

    var viewPos = [];
    var projPos = [];

    var canvas = $document[0].getElementById(Human.CANVAS_ID);

    var viewMat, projMat, canvasRect, x, y, w;

    Hotspots.getViewPos = function (worldPos) {
      viewMat = Human.renderer.getViewMat();

      if($window.vec3) {
        viewPos = $window.vec3.transformMat4(viewPos, worldPos, viewMat);
      } else {
        viewPos = Human.math.transformPoint3(viewMat, worldPos);
      }
    ...
    Hotspots.toCanvasSpace = function (viewPos) {
      ...
      return [
        (1 + x / w) * canvasRect.width / 2,
        (1 - y / w) * canvasRect.height / 2
      ];
    };

    Hotspots.buildSrcData = function (moduleId) {
      var module = Human.modules.modules[moduleId];
      var __id = 0;

      if(module.chapters && module.chapters.length) {

        angular.forEach(module.chapters, function (chapter, chapterIndex) {
          // Build from hotspots property
          if(chapter.hotspots && chapter.hotspots.length) {

            chapterIndex = chapterIndex.toString();

            if(!this.srcData.byChapter[chapterIndex]) {
              this.srcData.byChapter[chapterIndex] = [];
            }

            angular.forEach(chapter.hotspots, function (hotspot) {
              this.srcData.byChapter[chapterIndex].push(hotspot);

              if(hotspot.id) {
                this.srcData.byId[hotspot.id] = hotspot;
              }

              // Attach private internal ids to srcData hotspots
              hotspot.__id = __id.toString();

              this.srcData.all[hotspot.__id] = hotspot;
    ...
    Hotspots.getFromSrcData = function (srcData) {
      for (var i = 0; i < this.hotspots.length; i++) {
        if(this.hotspots[i].__id === srcData.__id) {
    ...
    Hotspots.clearSrcData = function () {
      angular.forEach(this.srcData, function (store) {
    ...
    Hotspots.clear = function () {
      this.hotspots.length = 0;
    };

    Hotspots.create = function (data) {
      var hotspot = angular.copy(data);

      // Shown state
      if(!angular.isDefined(hotspot.shown)) {
        hotspot.shown = true;
      }

      // Get pos from objectId if necessary
      if(!hotspot.pos && hotspot.objectId) {
        var object = Human.scene.objects[hotspot.objectId];

        if(object) {
          hotspot.pos = Human.scene.objects[hotspot.objectId].getCenter();
        }
      }

      // Rules for hotspot "type" property
      var type = $filter('camelCaseNormalize')(hotspot.type);
      ...
      if(type && HOTSPOT_TYPES[type]) {
        ...
        if(hotspot.className) {
          hotspot.className = [typeData.className, hotspot.className].join(' ');
        } else {
          hotspot.className = typeData.className;
        }

        // Override with type's template
        hotspot.template = typeData.template;

        if(typeData.title) {
          hotspot.title = typeData.title;
        }

      }

      this.createElement(hotspot);
    ...
    Hotspots.createElement = function (hotspot) {
      var tagName = 'human-hotspot';
      var hotspotEl = hotspot.element = $document[0].createElement(tagName);

      if(hotspot.id) {
        hotspotEl.id = hotspot.id;
      }

      if(hotspot.className) {
        hotspotEl.className = hotspot.className;
      }

      if(hotspot.template) {
        hotspotEl.innerHTML = $interpolate(hotspot.template)(hotspot);
      }

      if(hotspot.actions) {
        hotspotEl.setAttribute('actions', hotspot.actions.join('|'));
      }

      if(hotspot.pos) {
        hotspotEl.setAttribute('world-pos', hotspot.pos.join(','));
      }

      if(hotspot.objectId) {
        hotspotEl.setAttribute('object-id', hotspot.objectId);
      }

      if(angular.isObject(hotspot.data)) {
        var key, value;

        angular.forEach(hotspot.data, function (_value, _key) {
      ...
      angular.element(hotspotEl).data('hotspot', hotspot);

      return hotspotEl;
    };

    Human.events.on('modules.activating', function (data) {
      Hotspots.clearSrcData();
      Hotspots.clear();

      Hotspots.buildSrcData(data.moduleId);
    });
...
.directive('humanHotspots', ['$compile', 'HumanActions', 'Hotspots', function ($compile, HumanActions, Hotspots) {
  return {
    restrict: 'E',
    scope: true,
    link: function (scope, element) {
      ...
      Human.events.on('timeline.chapters.activated', function (data) {
        ...
        // Clear out existing hotspots
        element.empty();
        Hotspots.clear();

        // Create new instances from srcData
        var byChapter = Hotspots.srcData.byChapter;

        if(byChapter.hasOwnProperty(chapterIndex)) {

          angular.forEach(byChapter[chapterIndex], function (hotspotData) {
            hotspot = Hotspots.create(hotspotData);

            element.append(hotspot.element);
            $compile(hotspot.element)(scope);
          });
        }
...
.directive('humanHotspot', ['Human', 'HumanActions', 'Hotspots', function (Human, HumanActions, Hotspots) {
  ...
  return {
    ...
    link: function (scope, element, attrs) {
      ...
      var hotspot = element.data('hotspot'); // Reference to hotspot

      var worldPos = attrs.worldPos.split(',');
      var objectId = attrs.objectId || null;

      // Various possible states
      var inView = true;
      var objectShown = true;

      var viewPos, canvasPos;

      // Add methods to hotspot

      // TODO: Should this be done here?

      // Should there even be a humanHotspot directive?
      // Doesn't seem to be a point. Hotspot prototype?

      // Set property and update DOM element display
      hotspot.setShown = function (shown) {
        hotspot.shown = shown;
        updateDisplay();
      };

      var updateDisplay = function () {
        var show = hotspot.shown && inView && objectShown;

        hotspot.element.style.display = show ? 'block' : 'none';
      };

      var updateObjectShown = function (data) {
        var update = data.enabledObjectsUpdate;
        var object = Human.scene.objects[objectId];

        if(!object || !update.hasOwnProperty(objectId)) {
          return;
        }

        objectShown = update[objectId];
        updateDisplay();
      };

      var updatePos = function (worldPos) {
        viewPos = Hotspots.getViewPos(worldPos);
        canvasPos = Hotspots.toCanvasSpace(viewPos);

        hotspot.element.style.transform =
          'translate(' + canvasPos[0] + 'px,' + canvasPos[1] + 'px)';

        inView = viewPos[2] <= 0;
        updateDisplay();
      };

      var boundUpdatePos = updatePos.bind(null, worldPos);
      var boundUpdateObjectShown = updateObjectShown.bind(null);

      element.on('click', function () {
        var actions = element.attr('actions');

        if(actions) {
          // Build context from id collection and 'self' keyword
          var byId = Hotspots.srcData.byId;
          var _self = { self: Hotspots.srcData.all[hotspot.__id] || {} };

          var context = angular.extend({}, byId, _self);

          HumanActions.parse(actions, context);
        }
      });

      Human.events.on('camera.updated', boundUpdatePos);
      Human.events.on('canvas.resized', boundUpdatePos);

      if(objectId) {
        Human.events.on('scene.objectsShown', boundUpdateObjectShown);
      }

      boundUpdatePos();

      // Unbind Human events when hotspot is removed from DOM
      element.on('$destroy', function () {
        hotspot = undefined;

        Human.events.off('camera.updated', boundUpdatePos);
        Human.events.off('canvas.resized', boundUpdatePos);

        if(objectId) {
          Human.events.off('scene.objectsShown', boundUpdateObjectShown);
        }
...
.factory('ObjectOpacities', ['Human', 'SceneObjects', function (Human, SceneObjects) {
  ...
  return {
      ...
      objectIds.forEach(function (objectId) {
        ...
        for (i = 0; i < _leaves.length; i++) {
          originalState[ _leaves[i].objectId ] = {
            transparent: _leaves[i].transparent,
    ...
    // Reset opacity / transparent props on all leaf objects to their
    // Original state
    ...
    resetScene: function () {
      ...
      for(var id in originalState) {
        if(originalState.hasOwnProperty(id)) {
          ...
          if(state.hasOwnProperty('transparent')) {
            object.setTransparent(state.transparent);
          }
  ...
  return {
    ...
    link: function (scope, element, attr) {
      ...
      var setShow = function () {
        scope.show = configShow && assetsAttached;
      };
