...
.factory('HotspotData', ['$timeout', 'Human', 'currentModule', function ($timeout, Human, currentModule) {
  ...
  // TODO: Update other collections if modifying chapterScope or name
    ...
    */
    ...
    Hotspot.prototype._buildType = function () {
      ...
      if(type && HOTSPOT_TYPES[type]) {
        ...
        if(this.className) {
          this.className = [typeData.className, this.className].join(' ');
        } else {
          this.className = typeData.className;
        }
    ...
    Hotspot.prototype._getViewAngle = function () {
      ...
      dot = $window.vec3.dot(eyeVec, this.dir);
    ...
    Hotspot.prototype.doActions = function () {
      ...
      var context = angular.extend({}, byName, _self);

      HumanActions.parse(this.actions, context);
    };
    ...
    var opacity, angleDiff, fadeRange;
    ...
    Hotspot.prototype._setOpacity = function () {
      ...
      if(this._viewAngle >= FADE_START_ANGLE) {
        ...
        opacity = $window.Math.max(1 - (angleDiff / fadeRange), 0);
      } else {
        opacity = 1;
      }
    ...
    Hotspot.prototype._onPick = function () {
      this.doActions();

      // Finally, notify
      this._pickUpdate.id = this.id;

      Human.events.fire('hotspots.picked', this._pickUpdate);
    };

    Hotspot.prototype.bindDOMHandlers = function () {
      this._boundOnPick = this._onPick.bind(this);
      this.element.addEventListener('click', this._boundOnPick);
    };

    Hotspot.prototype.unbindDOMHandlers = function () {
      if(angular.isFunction(this._boundOnPick)) {
        this.element.removeEventListener('click', this._boundOnPick);
        this._boundOnPick = undefined;
      }
    };
...
.factory('HumanActions', ['$timeout', '$parse', function ($timeout, $parse) {
  ...
  var doActions = function (actions, customContext, callback) {
    ...
    if(actionsMap.hasOwnProperty(_instruction)) {
      ...
      actionObj.fn(_instruction, data, _callback);
...
.run(['$window', '$timeout', 'Human', 'currentModule', 'HumanBackground', 'HumanActions', 'Hotspot', function (
  ...
  Human.events.on('modules.activating', function () {
    var chapters = currentModule.data.chapters || [];
  ...
  var tweenOpacity = function (instruction, data, callback) {
    ...
    doTween(data[0], data[1], data[2] * 1000, onUpdate, callback);
  };

  HumanActions.define(['tweenOpacity'], tweenOpacity, context);

  // Background color tween action
  var tweenBackground = function (instruction, data, callback) {
    var updates = [
      function (values) {
  ...
  HumanActions.define(backgroundActions, tweenBackground, context);
  ...
  var playAction = function (instruction, data, callback) {
    ...
    if(!angular.isDefined(data)) { // No data, play from wherever we are now
      ...
      params.startChapterId = getChapterObject(data).id;
      params.numChapters = 1;

      var animation = getChapterObject(data).animation;
  ...
  var hotspotsAction = function (instruction, data, callback) {
    ...
    for (var i = 0; i < data.length; i++) {
      var hotspot = Hotspot.get(data[i].id);
...
.factory('HumanTimeline', ['Human', function (Human) {
  ...
  return {
    ...
    playChapter: function (targetChapter) {
      Human.timeline.play({
    ...
    handleChapter: function (targetChapter) {
      var currentBranch = Human.timeline.activeRoot._nowBranch;
      var currentChapter = Human.timeline.chapters[currentBranch.id];

      var diff = targetChapter.index - currentChapter.index;

      if(Math.abs(diff) === 1) {
        Human.timeline[diff === 1 ? 'next' : 'prev']();
      } else if(!!targetChapter.loop) {
        this.playChapter(targetChapter);
      } else {
...
.directive('humanPlayPause',
  ['$timeout', 'assetAttachClass', 'HumanTimeline', function ($timeout, assetAttachClass, HumanTimeline) {
    return {
      restrict: 'E',
      scope: true,
      ...
      link: function (scope, element) {
        ...
        // Need to call isPlayable async here
        ...
        element.on('click', function (e) {
          if(/chapter/.test(e.target.className)) {
            ...
            if(mode === 'scrubber') { // Scrubber
              HumanTimeline.playChapter(targetChapter);
            } else { // Tour
              HumanTimeline.handleChapter(targetChapter);
            }
          }
        });
...
.directive('humanScrubber',
  ['$filter', '$timeout', 'Human', 'assetAttachClass', 'HumanTimeline', function ($filter, $timeout, Human, assetAttachClass, HumanTimeline) {
    return {
      restrict: 'E',
      scope: true,
    ...
    link: function (scope, element, attrs) {
      ...
      element.on('click', function () {
        if(!disabled) {
          ...
          var targetBranch = branches[currentIndex + dir];

          HumanTimeline.handleChapter(Human.timeline.chapters[targetBranch.id]);
        }
      });
