  ...
  ['$timeout', 'Human', 'Process', function ($timeout, Human, Process) {
    ...
    return {
      ...
      link: function (scope, element) {
        ...
        var show, stateProgresses, processModuleId;

        var buildStateProgresses = function (stateLength) {
          stateProgresses = [];

          for (var i = 0; i < stateLength; i++) {
            stateProgresses.push(0);
          }
        };

        var updateStateProgresses = function(progress) {
          for (var i = 0; i < stateProgresses.length; i++) {
            if(progress > stateProgresses[i]) {
              stateProgresses[i] = progress;
              break;
            }
          }
        };

        var getTotalProgress = function () {
          var total = 0;

          for (var i = 0; i < stateProgresses.length; i++) {
            total += stateProgresses[i];
          }

          return total / stateProgresses.length;
        };
        ...
        // Fired right before processes.started.
        // We need this event to get moduleId
        Human.events.on('modules.activating', function (params) {
          processModuleId = params.moduleId;
        });
        ...
        Human.events.on('processes.started', function (params) {
          ...
          $timeout(function () {
            ...
            var processModule = Human.modules.modules[processModuleId];
            var modelIds = processModule.modelIds;
            var stateLength = Array.isArray(modelIds) ? modelIds.length : 1;

            buildStateProgresses(stateLength);
        ...
        Human.events.on('processes.updated', function (params) {
          ...
          if(process) {
            ...
            if(params.progress) {
              // Sort single stream into state buckets
              updateStateProgresses(params.progress);
              process.progress = getTotalProgress();
            }
          }
      ...
      var postLink = function (scope, element) {
        scope.$watch('treeData', function (n) {
          if(n) {
            ...
            // Store li elements for getting
            Tree.buildLis(element.find('li'));
          }
        });
      };
