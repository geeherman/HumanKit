...
.filter('Bookmark', function () {
  ...
  var getShowObjects = function (prefix, objects) {
    var showObjects = {};
  ...
  return function (bookmark) {
    ...
    if(!!bookmark.modules) {
      ...
      if(angular.isArray(activeModules) && activeModules.length) {
        ...
        for (var i = 0; i < activeModules.length; i++) {
          ...
          if(match) {
            baseIndex = i;
            basePrefix = new RegExp('^' + match[1]);
            break;
          }
        }

        // Ensure base anatomy is first module in array
        if(activeModules.length > 1 && baseIndex !== 0) {
          activeModules.unshift(activeModules.splice(baseIndex, 1)[0]);
        }

        if(!!basePrefix) {
          // Extract enabled base anatomy objects
          // Emulates search/bookmarks/data service
          showObjects = getShowObjects(basePrefix, bookmark.objects);

          if(Object.keys(showObjects).length === 0) {
            // Remove base from activeModules if no objects are enabled
            activeModules.shift();
          }
        }
...
.provider('BookmarkTour', ['BOOKMARK_TOUR_BASE_STATE', function (BOOKMARK_TOUR_BASE_STATE) {
  ...
  var $get = function ($filter, $timeout, $q, Human) {
    ...
    var _needsNewBase = function (showObjects) {
    ...
    var _reloadBaseAnatomy = function (id, showObjects) {
      var deferred = $q.defer();
      ...
      Human.modules.db.loadModuleDefinitions(libId, [id], function () {
        ...
        module.streamObjects = showObjects;
        module.showObjects = {};
    ...
    var _preRestore = function (capture, showObjects) {
      var deferred = $q.defer();

      var hasShowObjects = Object.keys(showObjects).length > 0;
      var needsNewBase = _needsNewBase(showObjects);

      if(hasShowObjects && _config.reloadBaseAnatomy && needsNewBase) {
        var id = capture.modules.activeModules[0];

        $timeout(function () {
          _reloadBaseAnatomy(id, showObjects).then(deferred.resolve);
        });
      } else {
        deferred.resolve();
      }
    ...
    state.restore = function (i, callback) {
      ...
      if(capture) {
        ...
        var result = $filter('Bookmark')(capture);
        capture = result.bookmark;

        _preRestore(capture, result.showObjects)
          .then(function () {
            $timeout(function () {
  ...
  return {
    ...
    link: function (scope) {
      ...
      Human.events.on('modules.deactivated', function () {
        if(Human.scene.rootObjects.length === 0) {
          scope.sceneObjects = [];
          scope.show = false;
        } else {
          init(); // Reinitialize
