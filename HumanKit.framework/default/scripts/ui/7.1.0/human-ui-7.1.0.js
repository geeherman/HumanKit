...
.directive('humanAnnotations', ['Human', function (Human) {
  return {
    ...
    link: function (scope, element, attr) {
      ...
      scope.tooltipText = attr.tooltipText || 'Toggle Annotations';
...
.directive('humanCrossSectionControls', ['$parse', 'Clip', function ($parse, Clip) {
  return {
    restrict: 'E',
    scope: true,
    ...
    link: function (scope, element, attr) {
      ...
      if(angular.isObject(scope.tooltipText)) {
        scope.tooltipText = angular.extend(tooltipText, scope.tooltipText);
      } else {
...
.directive('humanCrossSection', ['$parse', 'Clip', 'Human', function ($parse, Clip, Human) {
  return {
    restrict: 'E',
    scope: true,
    ...
    link: function (scope, element, attr) {
      ...
      scope.tooltipText = { toggle: 'Cross Section' };

      if(attr.tooltipText) {
        if(attr.tooltipText[0] === '{') { // Quick and dirty object test
          angular.extend(scope.tooltipText, $parse(attr.tooltipText)({}));
        } else {
          scope.tooltipText.toggle = attr.tooltipText;
        }
      }
...
.directive('humanDissect', ['$parse', 'Human', function ($parse, Human) {
  return {
    ...
    link: function (scope, element, attr) {
      ...
      scope.tooltipText = {
        dissect: 'Dissect [Ctrl + X]',
        undo: 'Undo Dissect [Ctrl + Z]'
      };
      ...
      if(attr.tooltipText) {
        if(attr.tooltipText[0] === '{') { // Quick and dirty object test
          ...
          scope.tooltipText.dissect = attr.tooltipText;
        }
      }
...
.directive('humanXray', ['Human', function (Human) {
  return {
    ...
    link: function (scope, element, attr) {
      scope.tooltipText = attr.tooltipText || 'X-Ray Mode [T]';
...
.directive('humanIsolate', ['Human', function (Human) {
  return {
    ...
    link: function (scope, element, attr) {
      scope.tooltipText = attr.tooltipText || 'Isolate Mode [I]';
...
angular.module('templates/cross-section/panel.html', []).run(['$templateCache', function($templateCache) {
  $templateCache.put('templates/cross-section/panel.html',
    ...
    '  uib-tooltip="{{ tooltipText.toggle }}"></button>\n' +
    '\n' +
...
angular.module('templates/dissect/panel.html', []).run(['$templateCache', function($templateCache) {
  $templateCache.put('templates/dissect/panel.html',
    ...
    '  toggle-model="shown"\n' +
    '  uib-tooltip="{{ tooltipText.dissect }}"></button>\n' +
    '\n' +
    '<button\n' +
    '  class="tool dissect-undo"\n' +
    '  undo-dissect\n' +
    '  uib-tooltip="{{ tooltipText.undo }}"></button>');
}]);
...
angular.module('templates/modes/isolate.html', []).run(['$templateCache', function($templateCache) {
  $templateCache.put('templates/modes/isolate.html',
    ...
    '  uib-tooltip="{{ tooltipText }}"></button>');
}]);
