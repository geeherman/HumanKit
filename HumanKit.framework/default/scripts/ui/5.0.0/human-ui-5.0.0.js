...
.factory('Human', ['MIN_HUMAN_VERSION', function (MIN_HUMAN_VERSION) {
  var versions = [];
...
.filter('extendConfig', ['CONFIG_ALIASES', 'CONFIG_GROUPS', function (CONFIG_ALIASES, CONFIG_GROUPS) {
  return function (dest, src) {
    ...
    // Extend individual properties, overwriting group settings
...
.factory('ConfigCreator',
  ['BASE_CONFIG', 'Human', 'uiComponents', 'extendConfigFilter', 'convertTagsFilter', 'uiParamsFilter', function (BASE_CONFIG, Human, uiComponents,
    ...
    return {
      ...
      init: function () {
        ...
        Human.events.on('modules.activating', this._extend.bind(this));
      },
      ...
      _extend: function (params) {
        ...
        var moduleConfig = module.ui || {};

        // Extend defaults with module's ui prop
        extendConfigFilter(config, moduleConfig);
        // Backwards compatibility with tags for module
        convertTagsFilter(config, module.tags);
        // Finally extend config with search params in url
        extendConfigFilter(config, this.params);
        // Make globally available
        uiComponents.config = config;
      }
...
.value('uiComponents', {}) // Store configurations here

.run(['$rootScope', 'ConfigCreator', 'uiComponents', function ($rootScope, ConfigCreator, uiComponents) {
  ConfigCreator.init();
  // Put on scope for directives to access
  $rootScope.uiComponents = uiComponents;
}]);
...
.directive('humanLoadProgress', ['INITIAL_STATUS_TEXT', 'Human', 'Process', function (INITIAL_STATUS_TEXT, Human, Process) {
  return {
    restrict: 'E',
    scope: true,
    ...
    link: function (scope) {
      scope.$watch('uiComponents.config.playPause', function (n) {
        if(scope.uiComponents.config) {
          ...
          scope.show = !scope.uiComponents.config.scrubber && !!n;
