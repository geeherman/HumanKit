...
.directive('humanCameraMode', ['$timeout', 'Human', function ($timeout, Human) {
  return {
    ...
    link: function (scope, element) {
      ...
      properties.resubscribe(subId, 'input.touchOrbitPan');
...
.directive('humanInfo', ['$window', '$timeout', '$filter', 'Info', function ($window, $timeout, $filter, Info) {
        ...
        var update = function (_timeFrame) {
          ...
          if(!listening) {
            ...
            Human.events.on('timeline.playing', function (params) {
              ...
              $timeout(scope.$digest.bind(scope)); // For performance
            });
          }
        };
