      ...
      link: function (scope, element) {
        ...
        Human.events.on('modules.activate.finish', function () {
          if (firstActivate) {
            ...
            // Create first chapter's hotspots
            createChapterHotspots('0');
        ...
        Human.events.on('timeline.chapters.activated', function (data) {
          if (!firstChapter) {
            ...
            createChapterHotspots(chapterIndex);
          }
...
.constant('HOTSPOT_TEMPLATES', {
  circle: '<div class="outer"><div class="inner">{{ symbol }}</div></div>',
  label: '<h1>{{ title }}</h1>'
})
...
.factory('Hotspot',
  ['$window', '$filter', '$interpolate', 'HOTSPOT_TYPES', 'HOTSPOT_CLASSES', 'HotspotData', 'Human', 'HumanActions', 'currentModule', 'doTween', function (
    $window, $filter, $interpolate,
    HOTSPOT_TYPES, HOTSPOT_CLASSES,
    HotspotData, Human, HumanActions, currentModule, doTween
  ) {
    ...
    */
    ...
    Hotspot.prototype._buildType = function () {
      ...
      if (type && HOTSPOT_TYPES[type]) {
        ...
        if (this.className) {
          ...
          var classes = typeData.classes.slice();
          var currentClasses = this.className.split(' ');

          for (var i = 0; i < currentClasses.length; i++) {
            if (classes.indexOf(currentClasses[i]) === -1) {
              classes.push(currentClasses[i]);
            }
          }

          this.className = classes.join(' ');
        } else {
          this.className = typeData.classes.join(' ');
        }
    ...
    Hotspot.prototype._hasTransition = function (className) {
    ...
    var objectShown, display, inView, transition, doTransition;
    ...
    Hotspot.prototype.setDisplay = function (allowTransition) {
      ...
      if (this.element) {
        ...
        if (allowTransition) {
          ...
          doTransition = this._hasTransition(transition);
        }
...
.factory('HumanActions',
  ['$timeout', 'Human', 'HumanParser', 'currentModule', function ($timeout, Human, HumanParser, currentModule) {
  ...
  var id = 0; // Each action sequence will get an id for tracking
  ...
  var doActions = function (actions, customContext, callback) {
    ...
    if (!currentActions.hasOwnProperty(this.id)) {
      exit.call(null, callback, this.id, -1);
      return;
    }
    ...
    if (actions.length === 0) { // Exit with success
      ...
      exit.call(null, callback, this.id, 0);
      return;
    }
    ...
    if (actionsMap.hasOwnProperty(_instruction)) {
      ...
      actionObj.fn.apply(fnContext, _arguments);
  ...
  var exit = function (callback, id, code) {
    $timeout(callback.bind(null, { id: id, code: code }));
  };

  // Base action context is the module definition
  // Plus a couple of useful additions
  var baseContext = {};
  ...
  Human.events.on('timeline.chapters.activated', function (data) {
    ...
    baseContext.currentChapter = baseContext.chapters ?
      baseContext.chapters[data.newChapterIndex] : null;
  });
  ...
  var HumanActions = {
    ...
    parse: function () {
      ...
      actions = actions.slice(0); // Create a copy for consumption
      ...
      var firstAction = doActions.bind({ id: id }, actions, context, callback);

      // Call in next execution context to avoid running into
      // Chaining problems with engine callbacks
      $timeout.call(null, firstAction, 0);
...
.run(['$window', '$timeout', 'Human', 'HumanBackground', 'HumanActions', 'Hotspot', 'Media', 'doTween', function (
    $window, $timeout,
    Human, HumanBackground, HumanActions, Hotspot, Media, doTween
  ) {

  var context = HumanActions.baseContext;
  var metaData;
  ...
  var playAction = function () {
    ...
    if (bindStopped) {
      ...
      // The Human.events.once method is broken
      // Using options {once:true} for Human.events.on is also broken
      // Workaround it here
      var _callback = function () {
        callback();

        $timeout(function () {
          Human.events.off('timeline.stopped', _callback);
        });
      };

      Human.events.on('timeline.stopped', _callback);
    }

    Human.timeline[this.instruction](params);
  ...
  var doActionsAction = function (data) {
    HumanActions.parse(data, {}, this.callback);
  };

  metaData = { arity: 1 };

  HumanActions.define(['doActions'], doActionsAction, context, metaData);
  ...
  HumanActions.define(['playAudio'], playAudioAction, context, metaData);
  ...
  HumanActions.define(['pauseAudio'], pauseAudioAction, context, metaData);
}]);
...
.directive('humanInfo', ['$filter', 'Info', function ($filter, Info) {
  ...
  return {
    ...
    link: function (scope) {
      ...
      // Ensure an update on bookmark tour restores
      Human.events.on('bookmarks.restored', function () {
        scope.$digest();
      });
  ...
  var $get = function (Human, SceneObjects) {
    ...
    return {
      ...
      init: function (layers, initIndex, minOpacity) {
        ...
        newLayer = layer = relativeDiff = i = undefined;
...
.directive('humanObjectSlider',
  ['Human', 'ObjectOpacities', 'HumanParser', 'currentModule', function (Human, ObjectOpacities, HumanParser, currentModule) {
    return {
      restrict: 'E',
      ...
      link: function (scope, element, attr) {
        ...
        element.on(ObjectOpacities.newObjectsEvent, function () {
          ...
          setOffsets();
          // Set initial class after offsets are set
          setLayersClass(fraction);
        });
        ...
        scope.$watch('uiComponents.config.objectSlider', function (n) {
          // TODO: Alias objects prop with layers?
