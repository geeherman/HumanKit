...
.factory('currentModule', ['$window', 'Human', function ($window, Human) {
  var curModule = {
    id: null,
    data: null
  };

  Human.events.on('modules.create.start', function (data) {
    if (curModule.id !== data.moduleId) {
      curModule.id = data.moduleId;
      curModule.data = // Poor man's deep copy
        $window.JSON.parse($window.JSON.stringify(data.moduleData));
    }
  });
...
.factory('HotspotInfo', ['$window', 'Info', function ($window, Info) {
  ...
  var HotspotInfo = {
    ...
    setCurrent: function (hotspot) {
      ...
      // Side effects:
      // Update info panel only when under threshold
      if ($window.innerWidth < HotspotInfo.breakPoint) {
        if (this.current) {
...
.factory('Hotspot',
  ['$window', '$filter', '$interpolate', 'HOTSPOT_TYPES', 'HOTSPOT_CLASSES', 'HotspotData', 'HotspotInfo', 'Human', 'HumanParser', 'HumanActions', 'currentModule', 'doTween', function (
    ...
    var FULL_OPACITY = 1.0; // Defined in stylsheet, need to hardcode this here
    ...
    var canvas = $window.document.getElementById(Human.canvasId || Human.CANVAS_ID);
    ...
    Hotspot.prototype.setDepth = function (depth) {
      if (!this.element) {
        return;
      }

      if (depth === -1) {
        depth = Hotspot.hotspotDepths.length;
      }

      this.element.style['z-index'] = depth;
    };
    ...
    var postSetCanvasPos = function () {
      ...
      for (var i = 0; i < Hotspot.hotspotDepths.length; i++) {
        if (Hotspot.hotspotDepths[i] !== HotspotInfo.current) {
          Hotspot.hotspotDepths[i].setDepth(i);
        }
      }

      if (HotspotInfo.current) { // Maintain the 'current' hotspot on top
        HotspotInfo.current.setDepth(-1);
      }
    ...
    var containerEl = $window.document.getElementById(Human.containerId || Human.CONTAINER_ID);
...
.directive('humanInfo', ['$window', '$filter', '$sce', 'Info', function ($window, $filter, $sce, Info) {
    ...
    link: function (scope, element) {
      ...
      var setInfo = function () {
        ...
        if ((title || description) && !Info.state.bookmarkRestoring) {
          ...
          if (info.description) {
            info.description = angular.isString(info.description) ?
              $sce.trustAsHtml(info.description) : '';
          }
    ...
    return {
      ...
      link: function (scope, element, attr) {
        ...
        scope.$watch('uiComponents.config.objectSlider', function (n) {
          ...
          if (hasDataSrc) {
            layers = HumanParser.parse(n.dataSrc)(currentModule.data);
          }
...
.provider('HumanSnapshot', function () {
  ...
  var $get = function ($window, $document, $q, $timeout, Human) {
    ...
    var getSnapshot = function (params, callback) {
      ...
      var engineCanvas = Human.renderer.canvas.canvas;

      if (engineCanvas[0]) {
        engineCanvas = engineCanvas[0];
      }
