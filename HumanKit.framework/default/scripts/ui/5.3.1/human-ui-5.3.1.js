...
.provider('BookmarkTour', ['BOOKMARK_TOUR_BASE_STATE', function (BOOKMARK_TOUR_BASE_STATE) {
  ...
  var $get = function ($filter, $timeout, $q, Human) {
    ...
    var _preRestore = function (capture) {
      ...
      // Mask current scene before restoring
    ...
    var _postRestore = function () {
      ...
      // Collect the current enabled leaf objects
      angular.forEach(Human.scene.enabledObjects, function (object, id) {
        if(object.objects.length === 0) {
          enabledObjects[id] = true;
        }
      });
      ...
      $timeout(function () {
        Human.scene.setEnabledObjects({ objectIds: enabledObjects });
      });
    };
