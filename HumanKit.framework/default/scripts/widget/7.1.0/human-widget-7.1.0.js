...
(function (window, angular) {
  ...
  .config(['$uibTooltipProvider', 'BASE_CONFIG', 'CONFIG_ALIASES', function ($uibTooltipProvider, BASE_CONFIG, CONFIG_ALIASES) {
    ...
    angular.extend(BASE_CONFIG, { help: false });
  ...
  .directive('gaEvent', ['$parse', '$timeout', function ($parse, $timeout) {
    var gaData = ['oldTracker.send', 'event', 'Widget'];
...
(function (window) {
  ...
  var sendLoadChapterEvent = function (params) {
    var newIndex = params.newChapterIndex;
    var chapter = Human.timeline.chapterList[newIndex];

    if (chapter) {
      var title = chapter.displayName || 'unknown';
      var chapterName = (newIndex + 1) + ' ' + title;
  ...
  analytics.ga.init = function () {
    ...
    eventDimensions[DIMENSIONS_MAP.appName] = 'Default';
    ...
    Human.events.on('timeline.chapters.activated', function(params) {
      if(!widgetData.content.bookmark) {
        if(Human.timeline.chapterList.length > 1) {
          sendLoadChapterEvent(params);
        }
      }
    });
  };
  ...
  analytics.ga.sendEvent = function(eCategory, eAction, eLabel) {
    var eventDimensionsObj = analytics.ga.getEventDimensionsObject();

    window.ga('set', eventDimensionsObj);
    ...
    return {
      ...
      init: function () {
        ...
        if(widgetParams['pre'] === 'false') {
          var remove = backgroundImage.remove.bind(backgroundImage);

          Human.events.on('modules.activated', remove);
        }
...
.run(['widgetData', 'HumanSnapshot', function (widgetData, HumanSnapshot) {
  if(widgetData.support.pass) {
...
.directive('clickContainer',
  ['$timeout', 'widgetParams', 'BackgroundImage', 'Human', function ($timeout, widgetParams, BackgroundImage, Human) {
    return {
      restrict: 'E',
      scope: true,
      ...
      link: function (scope, element) {
        ...
        var showInteractButton = function () {
          if(buttonShown || widgetParams['pre'] === 'false') {
            return;
          }
          ...
          if(handHintMode) {
            ...
            Human.events.on('started', function () {
              ...
              // Auto fade after 5 seconds if specified
