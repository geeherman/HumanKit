...
.config(['$uibTooltipProvider', function ($uibTooltipProvider) {
  ...
  if(Modernizr.touch) {
    var triggerAttributes = ['tooltip-trigger', 'proxy-tooltip-trigger'];

    for (var i = 0; i < triggerAttributes.length; i++) {
      var attribute = triggerAttributes[i];
      var tooltips = document.body.querySelectorAll('[' + attribute + ']');
      angular.element(tooltips).attr(attribute, 'none');
    }
...
.directive('gaEvent', ['$parse', '$timeout', function ($parse, $timeout) {
  var gaData = ['send', 'event', 'Widget'];
  ...
  return {
    ...
    link: function (scope, element, attrs) {
      ...
      var _sendEvent = function (args) {
        $timeout(function () {
          var data = gaData.concat(args);
          window.ga.apply(null, data);
        });
      };
      ...
      element.off('click.ga-event');
      element.on('click.ga-event', sendEvent);
    }
  };
}])
...
.run(['$rootScope', 'widgetParams', 'BackgroundImage', function ($rootScope, widgetParams, BackgroundImage) {
  ...
  if(
    widgetParams['image-display'] === 'fallback' ||
    widgetParams['load-image-display'] === 'fallback'
  ) {
...
.run(['$rootScope', 'HumanBootstrap', 'setCopyrightImage', function ($rootScope, HumanBootstrap, setCopyrightImage) {

  $rootScope.$on('human.result', function (e, data) {
    if(data.pass) {
      setCopyrightImage('img/bhuman-logo-powered.png'); // Default path
...
.provider('HumanBootstrap', function () {
  ...
  var $get = [
    ...
    'BackgroundImage', 'HumanFiles', 'widgetParams', 'WebGLError'
  ];
  ...
  var HumanBootstrap = function (
    ...
    var loadScripts = function (scripts, ok) {
      ...
      var loadUI = function () {
        $ocLazyLoad.load(humanUI).then(ok);
      };
    ...
    return {
      init: function () {
        ...
        HumanScene.onInit  = HumanScene.onInit  || function () {};
        HumanScene.onReady = HumanScene.onReady || function () {};
        HumanScene.onError = HumanScene.onError || function () {};
...
.factory('setCopyrightImage', ['$rootScope', '$injector', function ($rootScope, $injector) {
  return function (copyrightImagePath) {
    $rootScope.$on('human.loaded', function () {
      var HumanSnapshot = $injector.get('HumanSnapshot');
      HumanSnapshot.config({
        copyrightImagePath: copyrightImagePath
      });
...
.directive('clickContainer', ['widgetParams', 'BackgroundImage', '$timeout', function (widgetParams, BackgroundImage,
  $timeout) {
  return {
    restrict: 'E',
    scope: true,
    template: '<button><span>{{ message }}</span></button>',
    link: function (scope, element) {
      var handHintMode = widgetParams['load-hint-hand'];

      scope.message = 'Click to Interact';
      if(handHintMode) {
        element.addClass('hand');
        scope.message += ' in 3D';
      }

      var fadeInteractButton = function() {
        $timeout(function() {
          hideInteractButton();
        }, 10000);
      };

      var hideInteractButton = function() {
        element.remove();
        BackgroundImage.remove();
      };
      ...
      var showInteractButton = function () {
        ...
        // In hand mode, show element only after human is ready
        if(handHintMode) {
          scope.$on('human.ready', function () {
            scope.show = true;
            BackgroundImage.remove();

            // Auto fade after 10 seconds if specified
            if(handHintMode === 'fade') {
              fadeInteractButton();
            }
          });
      ...
      scope.$watch('pass', function (n) {
        ...
        if(n && scope.backgroundImage.load || handHintMode) {
          showInteractButton();
        }
      });
    }
...
.factory('WidgetActivity',
  ['$window', '$parse', '$q', '$interval', 'widgetData', 'HumanUserAnalytics', 'HumanDeveloperActivity', function (
    $window, $parse, $q, $interval,
    widgetData, HumanUserAnalytics, HumanDeveloperActivity
  ) {

    // Default verification callbacks
    var apiSuccess = function () {
      if(!angular.isFunction(Human.rpc.log)) {
        return;
      }
      ...
      $interval(function () {
        if (Human.rpc.logPending()) {
          ...
          var activity = HumanDeveloperActivity.buildActivity(customProps);

          HumanDeveloperActivity.post(activity).then(Human.rpc.resetLog);
        }
      }, 2000);
    };
    ...
    var domainSuccess = angular.noop;
    ...
    var buildChildAnalytic = function (parentAnalytic, config) {
      ...
      var description = [
        config.status,
        angular.toJson(widgetData.params)
      ].join(' ');
    ...
    return {
      ...
      verifyAPI: function (config) {
        ...
        HumanDeveloperActivity
          .post(buildAPIActivity())
          .then(success, failure);
      },

      verify: function () { // Convienance shorthand without options
        // Verify domain
        this.verifyDomain();
