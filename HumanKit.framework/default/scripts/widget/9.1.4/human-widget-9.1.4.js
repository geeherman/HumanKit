...
(function (window, angular) {
  ...
  HumanWidget.data = {
    ...
    appProductKey: HumanWidget.params['app-productkey'] || null,
    appUserId: HumanWidget.params['app-userid'] || null
  };
  ...
  var defaultInitSequence = function (config) {
    ...
    // Pre-Configure Engine
    if (Human.workers){
      var WORKER_PATH = ['scripts', 'engine', Human.VERSION, 'workers'].join('/');
      Human.workers.setPath(WORKER_PATH);
    }
  ...
  .controller('MainController', ['$scope', 'widgetData', function ($scope, widgetData) {
    ...
    Human.events.on('modules.activate.finish', function() {
        $scope.tour = Human.timeline.activeRoot._chapters.length > 1;
        $scope.$digest();
      });
...
(function (window) {
  ...
  var BASE_ACTIVITY = {
    // App keys
    'app_productkey': widgetData.appProductKey || '',
    'app_userid': widgetData.appUserId || '',
    // User keys
    'app_apikey': widgetData.developerKey || '',
    'app_domain': widgetData.parentUrl.full,
    'app_activity': '',
    'app_activity_status': '',
    'app_activity_data': {},
    'platform': 'widget',
  ...
  for (key in widgetData.params) {
    if (widgetData.params.hasOwnProperty(key)) {
      if (key !== 'dk') {
        value = widgetData.params[key];
  ...
  var BASE_ANALYTIC = {
    ...
    'analytic_data': {
      'parent_analytic_identifier': widgetData.userAnalyticId || '',
      'referer_domain': widgetData.parentUrl.full
    },
  ...
  for (key in widgetData.params) {
    if (widgetData.params.hasOwnProperty(key)) {
      if (key !== 'uaid') {
        value = widgetData.params[key];
        BASE_ANALYTIC['analytic_data']['url_param_' + key] = value;
      }
    }
  }
  ...
  analytics.buildAnalytic = function (customProps, customDataProps) {
    ...
    HumanWidget.extend(analyticData, customDataProps);
  ...
  analytics.ga.init = function (dimensionValues) {
    ...
    if(contentType === 'module') {
      contentName = Human.modules.modules[widgetData.content.id].displayName;
    ...
    if (Human.mobile && Human.mobile.bundleid) {
      eventDimensions[DIMENSIONS_MAP.appDomain] = Human.mobile.bundleid;

      if (Human.mobile.iOS) {
        eventDimensions[DIMENSIONS_MAP.platformType] = 'iOS';
      } else {
        eventDimensions[DIMENSIONS_MAP.platformType] = 'Android';
      }
...
(function (window) {
  ...
  HumanWidget.buildManifest = function (_dependencies, _config) {
    ...
    // Add optional engine plugins
    if(HumanWidget.params.plugins) {
      var engineVersion = dependencies.humanEngine;
      var enginePath = [config.basePath, 'engine', engineVersion].join('/');

      var plugins = HumanWidget.params.plugins.split(',');

      for (var i = 0; i < plugins.length; i++) {
        plugins[i] =
          [enginePath, 'plugins', plugins[i], plugins[i] + '.js'].join('/');
      }
...
(function (window) {
  ...
  // Valid list of API interaction triggers
  var RPC_INTERACT_ENDPOINTS = [
    'bookmark.nextChapter',
    'bookmark.previousChapter',
    'bookmark.set',
    'camera.set',
    'camera.zoom',
    'scene.load',
    ...
    'timeline.pause',
    'timeline.play',
    'timeline.previousChapter',
    'timeline.set',
    'scene.picked',
    'hotspots.picked'
  ];

  // List of touch to interact params
  var TOUCH_SUPPRESS_PARAMS = [
    { name: 'initial', value: 'true' },
    { name: 'initial.button', value: 'true' },
    { name: 'initial.hand-hint', value: 'true' }
  ];
  ...
  var apiSuccess = function () {
    ...
    var apiCallSize = 0;
    var apiIntervalId = window.setInterval(function () {

      if (firstInteraction || !_config.firstInteraction.api) {
        window.clearInterval(apiIntervalId);
        return;
      }

      // Validate API entries, use last chunk to check
      var rpcLog = Human.rpc.getLog();
      var entries = rpcLog.slice(apiCallSize);
      apiCallSize = rpcLog.length;
      for (var i = 0; i < entries.length; i++) {
        var rpc = entries[i];
        var isValid = RPC_INTERACT_ENDPOINTS.indexOf(rpc.identifier) !== -1;
        if (isValid) {
          firstInteract({
            type: 'api'
          });
  ...
  // Handles verification failure for both uaid and dk
  ...
  var onIntersect = function(entries) {
    if (entries[0].intersectionRatio >= activity.intersectThreshold) {
      while (deferredEvents.length) {
  ...
  var logEvent = function (eventFn) {

    if ('IntersectionObserver' in window) {

      observer = new window.IntersectionObserver(onIntersect, {
        root: null,
        threshold: activity.intersectThreshold
      });

      deferredEvents.push(eventFn);
  ...
  var onLoad = function () {
    ...
    if (developer) {
      ...
      // Changes on top of currently generated base activity data
      analytic = analytics.buildDeveloperActivity({ 'app_activity': 'view' });
      analytics.postDeveloperActivity(analytic, domainSuccess, domainFailure);

    } else if (user) {

      analytic = analytics.buildAnalytic({ 'analytic_type': 'view' });

      if (userVerified) {
        analytics.postAnalytic(analytic, domainSuccess, domainFailure);
      } else {

        var event = analytics.postAnalytic.bind(
          null, analytic, domainSuccess, domainFailure
        );

        userDeferredEvents.push(event);
      }

    }
  };

  var firstInteract = function (e) {
    if (firstInteraction) {
      return;
    }

    if (!contentInteractive) { // Force an interactionc event in this case
    ...
    if (developer) {
      ...
      analytic = analytics.buildDeveloperActivity(
        { 'app_activity': 'interactionf' },
        { 'interactionf_source': e.type }
      );
      ...
      analytic = analytics.buildAnalytic(
        { 'analytic_type': 'interactionf' },
        { 'interactionf_source': e.type }
      );
    ...
    // Un-bind handlers
    window.document.removeEventListener('mousedown', firstInteract);
  ...
  var onContentInteractive = function () {
    ...
    if (developer) {
      ...
      analytic = analytics.buildAnalytic({ 'analytic_type': 'interactionc' });
  ...
  // Set analytics related config
  activity.setConfig = function (extensions) {
    for (var configName in _config) {
      if (extensions.hasOwnProperty(configName)) {
        var extensionConfig = extensions[configName];
        if (angular.isObject(extensionConfig)) {
          angular.extend(_config[configName], extensionConfig);
        } else {
          _config[configName] = extensionConfig;
        }
      }
    }
  };

  activity.initContentInteractive = function (Human) {
    var geometryLoaded = false;
    var contentAnalyticFired = false;

    Human.events.on('assets.load.progress', function (data) {
      if (data.assetType === 'geometry') {
        geometryLoaded = true;
      }
    });

    Human.events.on('assets.load.finish', function () {
      if (geometryLoaded && !contentAnalyticFired) {
        contentAnalyticFired = true;
        logEvent(onContentInteractive);
      }
    });
  };
  ...
  activity.verifyAPI = function (config) {
    ...
    var activity = analytics.buildDeveloperActivity(
      {
        'platform': 'widgetapi',
        'app_activity': 'init'
      },
      {
        'engine_version': window.Human.VERSION,
        'api_version': window.Human.API_VERSION
      }
    );

    analytics.postDeveloperActivity(activity, success, failure);
  };

  activity.intersectThreshold = 0.75;

  var observer = null;
  var deferredEvents = [];
  var userDeferredEvents = [];
  var userVerified = false;
  var firstInteraction = false;
  var contentInteractive = false;

  // If neither uaid or dk params are present, we default to 'developer'
  // The developer service allows the white-listing of certain domains
  // Without any other credentials
  var user = !!widgetData.userAnalyticId;
  var developer = !user;

  var targetEl;

  if (user) {
    analytics.getAnalytic(widgetData.userAnalyticId, function () {
      userVerified = true;

      while (userDeferredEvents.length) {
        userDeferredEvents.shift()();
      }
  ...
  var onTouchStart = function (e) {
    ...
    if (_config.firstInteraction.touch) {
  ...
  var onTouchEnd = function (e) {
    ...
    if (_config.firstInteraction.touch) {
      ...
      var pageYDelta = lastPageY - touch.pageY;
      var screenYDelta = lastScreenY - touch.screenY;
      // Detect if interacting or scolling past
      var frameScrolled = pageYDelta !== screenYDelta;
      if (!frameScrolled || !widgetData.embedded) {
        firstInteract(e);
      }
    }
  };
  ...
  window.document.addEventListener('mousedown', firstInteract);
  // Check if touch interaction suppressed
  var attachTouchInteraction = true;
  for (var i = 0; i < TOUCH_SUPPRESS_PARAMS.length; i++) {
    var config = TOUCH_SUPPRESS_PARAMS[i];
    if (widgetData.params[config.name] === config.value) {
      attachTouchInteraction = false;
      break;
    }
  }
  if (attachTouchInteraction) {
