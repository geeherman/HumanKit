...
(function (window) {
  ...
  var sendLoadChapterEvent = function (chapters, newIndex) {
    var chapter = chapters[newIndex];

    if (chapter) {
      var title = chapter.info.displayName || 'unknown';
    ...
    Human.events.on('timeline.chapters.activated', function(params) {
      if(!widgetData.content.bookmark) {
        ...
        if(chapters.length > 1) {
          sendLoadChapterEvent(chapters, params.newChapterIndex);
        }
      }
    });
  };
